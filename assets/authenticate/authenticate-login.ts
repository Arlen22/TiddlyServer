{
  window.addEventListener('load', () => {
    ...
    field.addEventListener('keyup', ev => {
      if (ev.key === 'Enter') (window as any).tiddlyserverLogin()
    })
  })
  ...
  ;(window as any).keyhandler = e => {
    console.log(e)
    if ((e && e.keyCode == 13) || e == 0) {
      ;(window as any).tiddlyserverLogin()
    }
  }
  ;(window as any).tiddlyserverLogout = async () => {
    let res = await fetch('/admin/authenticate/logout', {
      ...
      byID<HTMLDivElement>('success-line').innerHTML =
        '<li>' +
    ...
    ;(async function() {
      ...
      if (res.status >= 400) {
        if (res.statusText === 'INVALID_CREDENTIALS') {
          ...
          byID<HTMLDivElement>('error-line').innerHTML =
            '<li>' +
            new Date().toISOString().slice(11, 19) +
