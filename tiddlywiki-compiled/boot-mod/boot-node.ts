...
export function bootNode($tw) {
    ...
    */
    function loadPluginFolder(filepath, excludeRegExp?): Observable<[TiddlerFields, TiddlerFields[]]> {
        excludeRegExp = excludeRegExp || $tw.boot.excludeRegExp;
        ...
        return obs_stat()(filepath).switchMap(([err, stat]) => {
            ...
            const alltiddlers = pluginInfo.tiddlers || Object.create(null)
            pluginInfo.tiddlers = undefined;
    ...
    function loadPlugin(name, paths): Observable<never> {
        return findLibraryItem(name, paths).switchMap(pluginPath => {
            ...
            if (res) {
                ...
                //if (!pluginInfo.text) pluginInfo.text = "";
                const tiddler = new $tw.Tiddler(pluginInfo);
                tiddler.cache.tiddlers = pluginTiddlers;
                //pluginInfo.text = { tiddlers: pluginTiddlers };
