export interface ServerConfigSchema extends ServerConfigBase {
	...
	server: ServerConfigBase["server"] & {
		 ...
		 * which exports the object. Note that the initServer function will change this to a boolean 
		 * value indicating whether https is in use once inside TiddlyServer
 		 */
		https?: string | SecureServerOptionsSchema;
	}
}
...
export interface NewTreePathOptions_Auth {
	/** Authenticate */
	$element: "auth";
	/** list of keys from the auth list that can access this resource */
	authList: string[];
}
