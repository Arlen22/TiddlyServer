...
export function generateSettingsPage(settings: ServerConfig, level: number) {
...
${(item.valueOptions as string[]).map(e => `<option ${defValue === e ? "selected" : ""} value="${e}">${e}</option>`).join('\n')}
</select> ${description}
</fieldset>`;
	}
}
function treeFunction(defValue: any, keys: string[]) {
	let res = "";
